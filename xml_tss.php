...
if (file_exists($xml_file)) {
    ...
    foreach ($xml->xpath("//offers/offer") as $segment) {
        ...
        $sql = "delete from goods WHERE (`id` = ".$row["id"]."); insert into goods (id, URL, available, cost,cost2, category, image , name, description, `ext-description`,spec, video, documents, `spare-parts`) values(".$row["id"].", '".$str."', ".$row["available"].", '".$segment->price." ₽"."', '".$segment->price."', ".$segment->categoryId.", '".$segment->picture."', '".$segment->name."', '".$segment->description."', '".$description."', '".$specifications."', '"."<div class=\"elements\" style=font-size:18px>
